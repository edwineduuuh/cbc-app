...
function Toast({ show, message, type, onClose }) {
  ...
  return (
    <div
      className={`fixed top-5 right-5 z-50 flex items-center gap-3 px-5 py-4 rounded-2xl shadow-2xl text-white font-semibold text-sm transition-all ${type === "success" ? "bg-green-600" : "bg-red-600"}`}
    >
      {type === "success" ? (
      ...
      <div className="flex items-center gap-4">
        {[1, 2].map((s) => (
          <div key={s} className="flex items-center gap-3 flex-1">
            ...
            <div>
              <p
                className={`text-sm font-semibold ${step >= s ? "text-gray-900" : "text-gray-400"}`}
              >
                {s === 1 ? "Quiz Details" : "Pick Questions"}
              </p>
              <p className="text-xs text-gray-400">
                {s === 1 ? "Title, grade, settings" : "Select from your bank"}
              </p>
            </div>
            {s < 2 && (
              <div className="flex-1 h-1 rounded-full bg-gray-200 mx-2">
                <div
                  className={`h-full rounded-full bg-indigo-500 transition-all duration-500 ${step > s ? "w-full" : "w-0"}`}
                />
              </div>
            )}
          </div>
...
export default function CreateQuizPage() {
  ...
  const goStep2 = () => {
    ...
    if (!subject) {
      showToast("Please select a learning area", "error");
      return;
    }
    if (!grade) {
  ...
  const createQuiz = async () => {
    ...
    try {
      ...
      if (res.ok) {
        showToast(
          `Quiz "${title}" created with ${selectedIds.length} questions!`,
        );
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-indigo-200 mt-1">
                Bundle your questions into a quiz for students
              </p>
            </div>
                    ...
                    {[
                      {
                        value: "topical",
                        label: "Topical Quiz",
                        desc: "Focused on one topic/strand. Good for practice after a lesson.",
                      },
                      {
                        value: "exam",
                        label: "Exam / Test",
                        desc: "Covers multiple topics. Good for end-of-term assessments.",
                      },
                    ].map((opt) => (
                      <label
                        key={opt.value}
                        className={`flex gap-4 p-5 rounded-xl border-2 cursor-pointer transition-all ${quizType === opt.value ? "border-indigo-500 bg-indigo-50" : "border-gray-200 hover:border-indigo-300"}`}
                      >
                        <input
                          type="radio"
                          name="quizType"
                          ...
                          onChange={() => setQuizType(opt.value)}
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    <div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <span className="w-14 text-center py-2 bg-indigo-50 text-indigo-700 font-bold text-sm rounded-lg border border-indigo-200">
                          {duration}m
                        </span>
                      </div>
                    </div>
                    <div>
                      <label className="flex items-center gap-1.5 text-sm font-semibold text-gray-700 mb-1.5">
                        <Award className="w-4 h-4 text-indigo-500" /> Passing
                        Score (%)
                      </label>
                      <div className="flex items-center gap-3">
                        <input
                    ...
                    {[
                      {
                        ...
                        checked: isB2C,
                        set: setIsB2C,
                        label: "Students (public)",
                        desc: "Any student on the platform can find and take this quiz.",
                      },
                      {
                        key: "teacher",
                        checked: availableToTeachers,
                        set: setAvailableToTeachers,
                        label: "Teachers (library)",
                        desc: "Teachers can find and assign this quiz to their classes.",
                      },
                    ].map((opt) => (
                      <label
                        key={opt.key}
                        className={`flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all ${opt.checked ? "border-indigo-400 bg-indigo-50" : "border-gray-200 hover:border-gray-300"}`}
                      >
                        <div
                          className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${opt.checked ? "bg-indigo-600 border-indigo-600" : "border-gray-300"}`}
              ...
              <div className="col-span-2 space-y-4">
                ...
                <div className="bg-indigo-600 text-white rounded-2xl px-5 py-3 flex items-center justify-between text-sm">
                  ...
                  <span className="bg-white/20 rounded-lg px-3 py-1">
                    {selectedSubject?.name || "—"} · Grade {grade}
                  </span>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                  ...
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                    <p className="text-sm text-gray-500">
                      {loadingQ
                        ? "Loading…"
                        : `${displayedQuestions.length} question${displayedQuestions.length !== 1 ? "s" : ""} found`}
                    </p>
                    <div className="flex gap-2">
                      <button
                        onClick={selectAll}
                        disabled={loadingQ}
                        className="text-xs px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-lg font-semibold hover:bg-indigo-100 disabled:opacity-50"
                      >
                      ...
                      {selectedIds.length > 0 && (
                        <button
                          onClick={clearAll}
                          className="text-xs px-3 py-1.5 bg-gray-100 text-gray-600 rounded-lg font-semibold hover:bg-gray-200"
                        >
                          Clear
                        </button>
                      )}
                    ...
                    <div className="flex flex-col items-center justify-center py-24 gap-3">
                      ...
                      {questions.length === 0 && (
                        <Link
                          ...
                          className="mt-2 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-semibold hover:bg-indigo-700 transition"
                        >
                          Go to Questions →
                        </Link>
                      )}
                      ...
                      {displayedQuestions.map((q) => {
                        ...
                        return (
                          ...
                          >
                            {/* Checkbox */}
                            <div
                              className={`mt-0.5 w-5 h-5 rounded flex items-center justify-center border-2 shrink-0 transition-all ${selected ? "bg-indigo-600 border-indigo-600" : "border-gray-300 group-hover:border-indigo-400"}`}
                            >
                              {selected && (
                              ...
                              <div className="flex flex-wrap items-center gap-1.5 mt-2">
                                ...
                                {q.in_quizzes &&
                                  ...
                                  q.in_quizzes.map((qz) => (
                                    <span
                                      key={qz.id}
                                      className="text-xs px-2 py-0.5 bg-violet-50 text-violet-700 border border-violet-200 rounded-full font-medium flex items-center gap-1 max-w-[140px]"
                                    >
                                      <svg
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                  ...
                  <dl className="space-y-2.5 text-sm">
                    <div className="flex justify-between gap-2">
                      ...
                      <dd className="font-medium text-gray-900 text-right max-w-[170px] truncate">
                        {title}
                      </dd>
                    </div>
                    <div className="flex justify-between gap-2">
                  ...
                  <button
                    onClick={() => setStep(1)}
                    className="mt-4 w-full py-2 text-sm text-indigo-600 font-semibold hover:bg-indigo-50 rounded-lg transition"
                  >
                    ← Edit details
                  </button>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span
                      className={`text-sm font-bold px-3 py-1 rounded-full ${selectedIds.length > 0 ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-500"}`}
                    >
                  ...
                  {selectedIds.length === 0 ? (
                    <p className="text-sm text-gray-400 text-center py-6">
                      Click questions on the left to select them
                    </p>
                  ) : (
                  ...
                  >
                    ...
                    {creating
                      ? "Creating…"
                      : `Create Quiz (${selectedIds.length})`}
                  </button>
                </div>
              </div>
            </motion.div>
